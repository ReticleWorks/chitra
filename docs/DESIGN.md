...
## Deterministic core and persistent reasoning boundary

chitra delivers messages to, and observes the state of, sessions that are themselves driven by an LLM. Its queue, dispatch, evidence, and storage paths remain deterministic. `monitord` binds each transcript to one frozen goal, persists corrective actions and question results, and independently executes enrolled validators. It never writes the terminal; `dispatchd` remains the sole writer. Each goal freezes an aggressive, goal-scoped `AutonomyPolicy` with typed capabilities and limits.

Task decomposition and response generation are now part of the foreground Chitra responsibility when deterministic supervision cannot settle the next step. The foreground agent may investigate evidence, ask the lane for context, replan, and direct successive actions until the enrolled goal is complete. The policy authorizes those actions by typed grant, target, expiry, and limit. The default grants goal-scoped replanning, redesign, dependency, schema, hook, credential, authentication, security, irreversible action, and spending. A goal may freeze narrower limits. The user is needed only for a verified missing, expired, or over-limit grant, or a change to the frozen outcome. `monitord` and `dispatchd` retain their separate roles: monitord records and pursues; dispatchd is the sole terminal writer.

Transport timeouts and failed delivery attempts are evidence for the pursuit loop. They do not abandon an unfinished goal. The loop can change tactics and retry after inspecting current state. Completion requires the independent validators described below, not a clean pane, a self-report, or a fixed retry count.
...
Validators can be slow, so the monitor never runs them on its own pass when the lane provably cannot forge the result. When the lanes manifest binds the goal's lane to a declared `uid` different from Chitra's effective uid — the same check `lane_anchor` uses to decide whether `runuser` is required — the monitor queues the enrolled validators on a bounded `concurrent.futures` worker pool, with at most one run in flight per lane. The worker digests the lane's worktree immediately before and after each run (HEAD, the full `git diff HEAD` patch, and a blob hash of every untracked file), stores the same receipt, and records the exit code, the receipt's integrity digest, and both tree digests under `validation-runs/<session-sha256>/<required_receipt>.json`. The gate accepts a recorded result only when both recorded digests still equal the worktree digest at gate time; a record whose tree moved is stale, so the monitor re-queues the run on the next pass and the claim never passes on stale evidence. While a run is in flight the claim is neither passed nor disputed and the lane is never counted as an idle pass.

When the manifest is missing, the lane is unbound, the worktree cannot be digested, or the lane shares Chitra's OS user, a record file proves nothing — the lane could have written it itself. Those lanes keep the previous behavior: the validator runs synchronously at claim time and the gate re-executes it synchronously. A worker that fails before it can record is retried a bounded number of times, then the monitor falls back to the same synchronous run so the failure surfaces loudly instead of looping silently.
