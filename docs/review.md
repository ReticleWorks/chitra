...
## 2026-07-14 — adoption enumeration and close inventory gates (v0.8.2)

Goal adoption now has an independent deterministic review before persistence.
A drafted `done_when` must enumerate a determinable deliverable count or carry
a normative annex with stable source-item IDs. Required annex items receive a
conservative lexical coverage diff; explicit `carried` items remain close-time
obligations, while `descoped` items require a reason and operator ack. Aggregate
terms such as "representative", "some", and bare deliverable plurals are
linted from an extendable module-level vocabulary.

The serialized annex is part of `freeze_goal`'s hashed payload, and annex
changes must use `redirect`, so the old annex is retained in goal history and
the goal version advances. `chitra-goals close` now accepts delivered item IDs
and performs the matching inventory diff before deleting state. A required
item described as follow-on, out of scope, deferred, or future work remains an
automatic close failure unless an operator-acknowledged redirect descoped it.
