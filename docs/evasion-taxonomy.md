...
| `OVER_QUESTIONING` | clarifying question whose answer is verbatim present in context, or "should I proceed?" when scope is unambiguous (genuine irreversible-action confirmation is not this) | `NUDGE` | partial — `question_handler` answers routine frozen-goal questions deterministically and answers from the newest matching recorded ruling in the decisions log, citing its decision id; punting to the operator is reviewer code `deferred_to_operator` |
