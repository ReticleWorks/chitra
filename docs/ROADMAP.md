...
Kai's monitor but had no equivalent in chitra. One of the two — session↔goal
...
- **Crash recovery / checkpointing (still deferred)** — Kai's `checkpoint` /
...
of Kai's `enroll` / `list-goals` / `revise-goal` / `close-goal`. This stays
...
**Gate 1 — License question: decided.** `pyproject.toml` declares `license = "MIT"`, copyright held by Reticle Works (the maintainer) — the final license decision, confirmed by the operator and merged via PR #4. This gate is closed.
