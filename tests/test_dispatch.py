...
from chitra.dispatch import (
    ...
    OPERATOR_TYPES_IN_PANES_ENV_VAR,
    DispatchOrder,
    DispatchStatus,
    LaneLock,
    LaneLockError,
...
def user_turn_jsonl(text: str, *, with_followup: bool = True) -> str:
    ...
    user-role record with a later assistant/tool/progress record proving the
    turn actually started (see ``_structural_transcript_confirms``) -- a
...
def test_dispatch_to_tmux_reports_failed_when_composer_never_clears(tmp_path: Path) -> None:
    ...
    never SENT -- this is fix (1)'s post-paste submit check, distinct from
    the pre-dispatch draft-flush policy (fix (3))."""
    projects_root = tmp_path / "projects"
    projects_root.mkdir()
...
# --- draft-flush policy (CHITRA_OPERATOR_TYPES_IN_PANES) -------------------


def test_operator_types_in_panes_defaults_false_and_reads_truthy_values() -> None:
    assert operator_types_in_panes("") is False
    assert operator_types_in_panes("false") is False
    assert operator_types_in_panes("true") is True
    assert operator_types_in_panes("TRUE") is True
    assert operator_types_in_panes("1") is True


def test_dispatch_to_tmux_flushes_a_stuck_composer_by_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """On this fleet the operator never types into a pane, so a composer
    holding text at pre-dispatch time is always a failed prior delivery, not
    a draft to protect. By default (CHITRA_OPERATOR_TYPES_IN_PANES unset)
    dispatch_to_tmux submits it and proceeds with the new nudge."""
    monkeypatch.delenv(OPERATOR_TYPES_IN_PANES_ENV_VAR, raising=False)
    projects_root = tmp_path / "projects"
    session_dir = projects_root / "some-project"
    session_dir.mkdir(parents=True)
    transcript = session_dir / "abc123.jsonl"
    transcript.write_text(user_turn_jsonl("run the checks"), encoding="utf-8")

    stuck_pane = (
        "Claude Code output\n──────────────────────\n❯ a stuck prior delivery\n"
        "──────────────────────\n⏵⏵ accept edits on (shift+tab to cycle)\n"
    )
    idle_pane = (
        "Claude Code output\n──────────────────────\n❯ \n"
    ...
    def runner(cmd: list[str], *, timeout: int = 20) -> subprocess.CompletedProcess[str]:
        if cmd[:2] == ["tmux", "capture-pane"]:
            captures["n"] += 1
            return fake_completed(0, stuck_pane if captures["n"] == 1 else idle_pane, "")
        sent.append(cmd)
        return fake_completed(0, "", "")
    ...
    order = DispatchOrder(order_id="o1", session_ref="localhost:f3:0.0", nudge="run the checks")
    result = dispatch_to_tmux(
        order,
        runner=runner,
        input_runner=input_runner,
    ...
    assert result.status == DispatchStatus.SENT
    # The stuck draft was flushed with a bare Enter before the new nudge was
    # ever pasted -- proven by at least one send-keys Enter having fired
    # before/alongside the ordinary paste recipe's own Enter.
    assert any(c[:2] == ["tmux", "send-keys"] and c[-1] == "Enter" for c in sent)


def test_dispatch_to_tmux_flush_escalates_to_backend_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    """A stuck Codex composer that a bare Enter alone does not clear must
    escalate to the same kitty-Enter fallback the post-paste submit check
    uses, before the new nudge is ever pasted."""
    monkeypatch.delenv(OPERATOR_TYPES_IN_PANES_ENV_VAR, raising=False)
    stuck_pane = "• Ready for input\n\x1b[1m›\x1b[0m a stuck prior codex delivery\n  ? for shortcuts"
    idle_pane = "• Ready for input\n\x1b[1m›\x1b[0m \n  ? for shortcuts"
    captures = {"n": 0}
    sent: list[list[str]] = []
    ...
    def runner(cmd: list[str], *, timeout: int = 20) -> subprocess.CompletedProcess[str]:
        if cmd[:2] == ["tmux", "capture-pane"]:
            captures["n"] += 1
            # 1: pre-check (stuck). 2: after the bare flush Enter (still stuck --
            # Enter alone never submits Codex's kitty-protocol composer). 3+:
            # after the kitty fallback and the rest of the pipeline (cleared).
            return fake_completed(0, stuck_pane if captures["n"] <= 2 else idle_pane, "")
        sent.append(cmd)
        return fake_completed(0, "", "")
    ...
    assert any(_CODEX_KITTY_ENTER_SEQUENCE in c for c in sent)


def test_dispatch_to_tmux_stays_blocked_when_flush_cannot_clear_the_composer() -> None:
    """When neither a bare Enter nor the backend fallback ever clears the
    composer, the flush gives up and the original BLOCKED result stands --
    it never silently proceeds to paste over an unresolved composer."""
    stuck_pane = (
        "Claude Code output\n──────────────────────\n❯ a stuck prior delivery\n"
        "──────────────────────\n⏵⏵ accept edits on (shift+tab to cycle)\n"
    )
    ...
    def runner(cmd: list[str], *, timeout: int = 20) -> subprocess.CompletedProcess[str]:
        if cmd[:2] == ["tmux", "capture-pane"]:
            return fake_completed(0, stuck_pane, "")
        return fake_completed(0, "", "")
...
def test_dispatch_to_tmux_preserves_blocked_when_operator_types_in_panes_is_true(monkeypatch: pytest.MonkeyPatch) -> None:
    """CHITRA_OPERATOR_TYPES_IN_PANES=true preserves today's behavior exactly:
    no flush is ever attempted, and the pre-dispatch draft still blocks."""
    monkeypatch.setenv(OPERATOR_TYPES_IN_PANES_ENV_VAR, "true")
    calls: list[list[str]] = []
    stuck_pane = (
        "Claude Code output\n──────────────────────\n❯ a genuine operator draft\n"
        "──────────────────────\n⏵⏵ accept edits on (shift+tab to cycle)\n"
    )
    ...
    assert result.status == DispatchStatus.BLOCKED
    assert result.reason == "blocked: unsubmitted operator draft detected"
    # No flush attempted: the only command issued at all is the single
    # pre-dispatch capture-pane read.
    assert calls == [["tmux", "capture-pane", "-e", "-p", "-t", "f3:0.0", "-S", "-12"]]
