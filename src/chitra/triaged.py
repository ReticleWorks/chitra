...
def _critical_rules() -> tuple[tuple[str, re.Pattern[str]], ...]:
    """Build the full rule set, including a ``needs_operator`` rule that always
    matches ``you``/``operator``/``input`` and layers any configured
    ``CHITRA_OPERATOR_ALIASES`` on top (same convention as ``chitra.lane_read``)."""
    targets = "|".join(re.escape(value) for value in ("you", "operator", *env_csv(OPERATOR_ALIASES_ENV_VAR)))
    needs_operator = re.compile(rf"needs (?:{targets}|input)|waiting on (?:{targets})", re.I)
