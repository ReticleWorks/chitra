...
def _open_ask_heading_re() -> re.Pattern[str]:
    """Build the open-ask heading pattern, including any configured operator aliases.

    ``you`` and ``operator`` are always matched (the package-default, name-free
    case); ``CHITRA_OPERATOR_ALIASES`` layers specific operator names or
    aliases on top for deployments that want their own name recognized too.
    """
    targets = "|".join(re.escape(value) for value in ("you", "operator", *env_csv(OPERATOR_ALIASES_ENV_VAR)))
    return re.compile(
        rf"awaiting ruling|open (?:question|ask|decision)s?|decisions? (?:needed|for you)|"
        rf"need(?:s)? (?:{targets})|for (?:{targets}) to (?:decide|rule)",
        re.IGNORECASE,
    )
