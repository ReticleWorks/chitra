...
Neither is a status change: it only retires open_asks. A lane with no open
asks (a status-only review item — blocked, completion-disputed, ... with no
literal ask) has nothing for either endpoint to do, so both raise
LaneActionError(no_op=True) rather than reporting a false success — the
caller (app.py) turns that into 409, not 200.
"""
...
def answer_lane(state_dir: Path, lane_id: str, text: str) -> GoalRecord:
    ...
    return _resolve_ask(state_dir, lane_id, basis=text.strip())
