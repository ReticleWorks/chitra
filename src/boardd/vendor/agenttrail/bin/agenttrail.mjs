...
const server = http.createServer((req, res) => {
  ...
  } else if (u.pathname === '/escalations') {
    // ponytail: local patch — the orchestrator writes `escalations` into roster.json; the page polls this
    let e = {}; try { e = JSON.parse(fs.readFileSync(path.join(repo, 'roster.json'), 'utf8')).escalations || {} } catch {}
    res.writeHead(200, { 'content-type': 'application/json' }).end(JSON.stringify(e))
  } else if (u.pathname === '/answer' && req.method === 'POST') {
    // ponytail: local patch — loopback only, JSON {key, answer, at} under 16 KB, one JSON line appended to .agenttrail/answers.log
    if (!['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress)) { res.writeHead(403).end(); return }
    let body = ''
    req.on('data', c => { body += c; if (body.length > 16384) { res.writeHead(413).end(); req.destroy() } })
    req.on('end', () => {
      let d = null; try { d = JSON.parse(body) } catch {}
      if (!d || typeof d.key !== 'string' || typeof d.answer !== 'string' || typeof d.at !== 'string') { res.writeHead(400).end(); return }
      fs.mkdirSync(path.join(repo, '.agenttrail'), { recursive: true })
      fs.appendFileSync(path.join(repo, '.agenttrail', 'answers.log'), JSON.stringify({ key: d.key, answer: d.answer, at: d.at }) + '\n')
