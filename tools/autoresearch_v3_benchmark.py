#!/usr/bin/env python3
"""Chitra AutoResearch v3 benchmark (fixed specification).
...
Usage
-----
  python tools/autoresearch_v3_benchmark.py             # public eval
  python tools/autoresearch_v3_benchmark.py --json      # machine-readable
  python tools/autoresearch_v3_benchmark.py --selftest  # mutation proofs
  python tools/autoresearch_v3_benchmark.py --sealed /host/sealed/cases.py
...
MODULE_ALIAS = "autoresearch_v3_public"
WORKER_TIMEOUT_S = 45
TESTS_TIMEOUT_S = 600
...
def make_world(tag: str) -> World:
    root = Path(tempfile.mkdtemp(prefix=f"chitra-autoresearch-v3-{tag}-"))
...
# v3 is rebased on the reproduced AR-04 champion. The active v2 benchmark
# remains frozen at the original 67-point baseline.
_CLEAN_DIMENSION_SCORES = {"D1": 12, "D2": 10, "D3": 18}
...
def _run_dimension_probe(checkout: Path, key: str) -> int:
    command = [
        sys.executable,
        str(checkout / "tools" / "autoresearch_v3_benchmark.py"),
        "--internal-dimension",
        key,
    ]
...
def emit(report: dict, as_json: bool) -> None:
    if as_json:
        ...
        return
    print(f"== Chitra AutoResearch v3 :: {report.get('mode', '?')} evaluation ==")
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="autoresearch_v3_benchmark",
        description=("Fixed v3 AutoResearch benchmark for chitra.dispatchd "
